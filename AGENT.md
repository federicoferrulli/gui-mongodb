...
- `public/js/` — frontend modulare (vanilla JS, nessuna build, nessun framework). Stato in un singolo oggetto `state.js`; utility condivise in `utils.js`. L'entrypoint è `main.js`, che coordina i moduli:
  - `utils.js`: Helper vari (EJSON, stringhe, modali, socket, errori)
  - `dbtree.js`: Render della sidebar coi DB (e logica di fuzzy finding)
  - `schema-ops.js`: Modali di creazione/alter (DDL)
  - `grid.js`: Griglia dati, query SQL/Mongo e impaginazione
  ...
  - `live.js`: Logica di polling / socket watch
  - `autocomplete.js`: Autocomplete contestuale per filtri e ordinamenti
  - `socket.js` e `uml.js`. Il workspace ha tre viste (`setView`): **Dati** (griglia), **Dettagli** (statistiche/indici/schema) e **UML** (SVG generato in `uml.js`). Menu contestuale generico (tasto destro su database e collection nella sidebar).
